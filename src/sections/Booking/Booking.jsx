...
function Fld({label,children}) {
  return (
    <div className="fld">
      <style>{`.fld{display:flex;flex-direction:column;gap:.38rem}.fld label{font-size:.8rem;font-weight:600;color:var(--text)}.fld input,.fld select{width:100%;padding:.72rem 1rem;border:1.5px solid #E5E7EB;border-radius:10px;font-family:inherit;font-size:.92rem;color:var(--text);outline:none;background:white;transition:border-color .2s}.fld input:focus,.fld select:focus{border-color:var(--forest-light)}`}</style>
      <label>{label}</label>
      {children}
    </div>
  )
